...
import { TodayActionPanel } from "@/components/dashboard/today-action-panel"
import {
  calculateBalance,
  formatBalance,
...
export async function DashboardSummary() {
  ...
  const caloriesRemaining = Math.max(0, goals.TMB_KCAL - nutrition.calorias)
  const proteinRemaining = Math.max(0, goals.PROTEIN_G - nutrition.proteinas)
  const waterRemainingLiters = Math.max(0, goals.WATER_L - nutrition.aguaLitros)
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <TodayActionPanel
        ...
        waterRemainingLiters={waterRemainingLiters}
        hasRecentMeasurement={latest.peso != null}
      />
    </div>
  )
}
